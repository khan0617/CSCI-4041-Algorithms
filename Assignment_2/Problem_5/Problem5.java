...
public class Problem5 {
    ...
    private int[][] D;
    private int[][] newD;
    private GraphNode[] newVertexMatrix;
    private int completedVertices; //this field represents how many vertices have already had their "shortest paths" successfully solved for in the g array.

    public Problem5(GraphNode[] nodes, int numCompletedVertices){
        g = nodes;
        ...
        completedVertices = numCompletedVertices;
    }

    private void updateG(GraphNode[] nodes){ //pass in an array of the "new" vertices added to the graph.
        g = nodes;
    }

    private void setCompletedVertices(int num){
        completedVertices = num;
    }

    private void setNewDMatrix(int size){ //this method is meant to be called after the 'D' matrix has already been populated by JohnsonSquareMatrix().
        newD = new int[size][size];
        for(int i = 0; i < D.length; i++){ //copy over the data from D in newVertexMatrix.
    ...
    private void initializeGraph(){ //this method sets all of g's nodes' ".d" values to inf and clears the .pi pointers.
    ...
    private void Dijkstra(GraphNode start){ //graph has been initialized already. Queue is populated with all the vertices, starting node has a ".d" value of 0.
        ...
        while(queue.size() != 0){
            ...
            for(Map.Entry<GraphNode, Integer> entry : u.map.entrySet()){
                ...
                if(!(entry.getKey().getColor().equalsIgnoreCase("black")) && !entry.getKey().getFoundShortestPath()) { //if the key we're looking at is not colored black, add it to the queue
    ...
    private boolean JohnsonSquareMatrix(){
        boolean n = BellmanFordSquare(g.length - 1); //call BellmanFord on the last index of g, since in my implementation that is the "super node"
        ...
        for(int i = 0; i < g.length - 1; i++){
            ...
            for(int j = 0; j < g.length - 1; j++){ //populate the relevant row of the matrix, D, after un-weighting all the path values
                    if (j != i) {
                        ...
                        if(i >= completedVertices || j >= completedVertices){ //only update the spots relevant to the NEW vertices
                            //if i and j were both < completedVertices, we'd be needlessly update old D[i][j] values.
                            D[i][j] = g[j].getD();
                        }
                    } else { //j == i here
                        if(i >= completedVertices) {
    ...
    private boolean BellmanFordSquare(int startNodePosition){
    ...
    public static void main(String[] args){
        ...
        try{
            ...
            GraphNode[] newVertices = new GraphNode[0];
            boolean readingFirstLine = true;
            int lineCount = 0, numVertices = 0, newVertexCount = 0; //numVertices is the number of vertices in the original SQUARE matrix, not including "new vertices"
            ...
            while((s = reader.readLine()) != null){
                ...
                if(lineCount < numVertices){ //this means we're still reading the initial square matrix
                ...
                else{ //lineCount >= numVertices, so looking at "new" vertices now.
                    if(newVertexCount == 0){
                        newVertexCount = sArray.length - numVertices; //number of new vertices is equivalent to length of the current readLine string - numVertices
                        newVertices = new GraphNode[newVertexCount];
                        for(int i = 0; i < newVertexCount; i++){ //populate the newVertices[] GraphNode array
                            newVertices[i] = new GraphNode(i+numVertices); //if numVertices == 3, then newVertices[0] will be a GraphNode w/ its "vertexNum" attribute as 3
                        }
                    }

                    int index = lineCount - numVertices;
                    for(int i = 0; i < sArray.length; i++){ //now loop through the readLine and populate the adjacency maps for the new vertices.
                        int edgeWeight = Integer.parseInt(sArray[i]);
                        if(i < numVertices){ //create an edge between the squareMatrix GraphNode and the new vertex
                            newVertices[index].insertPair(g[i], edgeWeight);
                        }
                        else{ //i >= numVertices, so now we're looking at edge pairs only between new vertices.
                            if(edgeWeight != 0){
                                int adjIndex = i - numVertices;
                                newVertices[index].insertPair(newVertices[adjIndex], edgeWeight);
                            }
                        }
                    }
                }
            ...
            Problem5 P5 = new Problem5(g, 0); //call Johnson's alg on JUST the square matrix (NO new vertices yet).
            boolean n = P5.JohnsonSquareMatrix();
            String output = "";
            if(n == false){
                output = "Negative cycle";
            }
            else{
                output = P5.printSquareMatrix();
            }

            for(int i = 0; i < g.length - 1; i++){
                g[i].setFoundShortestPath(true); //the foundShortestPath variable allows us to NOT have to unnecessarily check the original nodes again in Dijkstra's alg.
            }

            if(newVertexCount >0){
                output += "\n";
                //now, we have to set up the new edge pairs in the old matrix, 'g', since this graph has undirected edges.
                for (int i = 0; i < newVertices.length; i++) {
                    GraphNode u = newVertices[i];
                    for (Map.Entry<GraphNode, Integer> entry : u.map.entrySet()) { //loop over each new vertex's HashMap (edge/vertex pairs)
                        GraphNode v = entry.getKey(); //currently, the newVertices are marked as having edges going to v, but we need to add the other direction in.
                        v.insertPair(u, entry.getValue());
                    }
                }
                GraphNode[] allNodes = new GraphNode[numVertices + newVertexCount + 1]; //+1 for the super node
                int count = 0;
                for(int i = 0; i < numVertices; i++){
                    allNodes[i] = g[i];
                    count++;
                ...
                for(int i = 0; i < newVertexCount; i++){
                    allNodes[i + count] = newVertices[i];
                }
                allNodes[allNodes.length - 1] = new GraphNode(2000000); //set allNodes' super node
                GraphNode superNode = allNodes[allNodes.length-1];
                for(int i = 0; i < allNodes.length - 1; i++){
                    GraphNode u = allNodes[i];
                    if(u.getFoundShortestPath() == false){
                        superNode.insertPair(u, 0);
                    }
                }
                //Problem5 P5v2 = new Problem5(allNodes, numVertices);
                //boolean x = P5v2.JohnsonSquareMatrix();
                P5.updateG(allNodes);
                P5.setCompletedVertices(numVertices);
                P5.setNewDMatrix(allNodes.length - 1);
                boolean x = P5.JohnsonSquareMatrix();
                if(!x){
                    output += "Negative cycle";
                }
                else{
